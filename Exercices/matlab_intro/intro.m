...
%
% Stefan Roth 
% <stefan DOT roth (AT) gris DOT informatik DOT tu-darmstadt DOT de>
% First version: 09/08/2003
% Last modified: 20/10/2014 (Stephan Richter)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Similar with "dbstop if naninf": it will stop if any nan or inf value 
% is processed. You can clear it with "dbclear if naninf"
...
m = 5000; n = 10000; A = ones(m, n); v = 2 * rand(1, n); 

A - v                               % will fail due to different dimensions
...
% and even faster, without replication
tic;
c = bsxfun(@minus, A, v);
toc


A = bsxfun(@rdivide, ...            % fast normalization
    A, sqrt(sum(A(:).^2)));
...
I = imread('car_bw.png');     % Read a PNG image
...
I3 = double(I2);              % Convert from uint8 to double, to allow
                              %   math operations
I4 = im2double(I2);           % same as above, but with scaling to [0,1]
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% things to look at (just some pointers):
% anonymous functions
% structs
% cells
% classes
